...
void main()
{
	...
	while(!kbhit())  //will keep printing on the console until the user presses a key on the keyboard
	{
		...
		//hour
		setcolor(0);
		arc(320,240,270,90-(t.ti_hour*6*5),55);  //arc(int x, int y, int start_angle,int end_angle, int radius)
		setcolor(YELLOW);
		getarccoords(&hr_arc);
		line(320,240, hr_arc.xend, hr_arc.yend);
		//minute
